...
router.get('/', async (req, res) => {
    try {
        ...
        if (cachedReferrals) {
            return res.status(200).json({ referrals: JSON.parse(cachedReferrals), source: 'cache' });
        }
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (cachedReferral) {
            return res.status(200).json({ referral: JSON.parse(cachedReferral), source: 'cache' });
        }
