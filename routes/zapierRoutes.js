...
/**
 * @route POST /api/zapier/member-joined
 * @desc Webhook endpoint to receive member joined data from Zapier/Skool
 * @desc Automatically creates PREMIUM subscription (type=2) with duration=1000 for new members
 * @access Public (but should be secured with a secret token in production)
 */
  ...
  try {
    ...
    if (existingUser) {
      ...
      return sendUpdateResponse(
        ...
        "User updated from Zapier webhook (member-joined)"
      );
    } else {
      // Find PREMIUM subscription (type = 2) BEFORE creating user
      const premiumSub = await Subscription.findOne({
      ...
      return sendCreateResponse(
        ...
        "User created from Zapier webhook (member-joined)"
      );
    }
  } catch (error) {
    console.error("❌ Error processing Zapier webhook (member-joined):", error);
    ...
    return sendInternalErrorResponse(
      res,
      "Error processing Zapier webhook (member-joined): " + error.message
    );
  }
});
